...
def plot_phase_fixed_V_H(sol):

    t = sol.t

    V_values = np.linspace(-100, 60, 25)
    h_values = np.linspace(0, 1.2, 25)
    ...
    plt.plot([], [], color='green', linestyle='dashed', label='dh = 0')
  
    plt.xlabel('Membrane Potential V (mV)')
    plt.ylabel('Gating Variable h')
    plt.title('Phase Diagram with Slope Field for V and h (V_H)')
    plt.legend()
    plt.grid(True)
    plt.show()
